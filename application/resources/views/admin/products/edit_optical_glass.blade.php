...
@section('content')
    ...
    <div class="white-box">
        ...
        <form method="post" action="" style="direction: rtl">
                ...
                <div class="col-md-6">
                    <select id="seller" class="form-control{{ $errors->has('seller') ? ' is-invalid' : '' }}" name="seller">
                        ...
                        @foreach($bonakdars as $bonak)
                            <option value="{{ $bonak['id'] }}" {{ (old('seller', $request->seller_id) == $bonak['id']) ? 'selected' : '' }}>{{ $bonak['name'] }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="type" class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}" name="type">
                        ...
                        @foreach($types as $key => $type)
                            <option value="{{ $type }}" data-key="{{ $key }}" {{ (old('type', $detail->type) == $type) ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="property" class="form-control{{ $errors->has('property') ? ' is-invalid' : '' }}" name="property">
                        ...
                        @foreach($properties as $property)
                            <option value="{{ $property }}" {{ (old('property', $detail->property) == $property) ? 'selected' : '' }}>{{ $property }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="refractive_index" class="form-control{{ $errors->has('refractive_index') ? ' is-invalid' : '' }}" name="refractive_index">
                        ...
                        @foreach($refractive_indexs as $refractive_index)
                            <option value="{{ $refractive_index }}" {{ (old('refractive_index', $detail->refractive_index) == $refractive_index) ? 'selected' : '' }}>{{ $refractive_index }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="anti_reflex_color" class="form-control{{ $errors->has('anti_reflex_color') ? ' is-invalid' : '' }}" name="anti_reflex_color">
                        ...
                        @foreach($anti_reflex_colors as $anti_reflex_color)
                            <option value="{{ $anti_reflex_color }}" {{ (old('anti_reflex_color', $detail->anti_reflex_color) == $anti_reflex_color) ? 'selected' : '' }}>{{ $anti_reflex_color }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="block" class="form-control{{ $errors->has('block') ? ' is-invalid' : '' }}" name="block">
                        ...
                        @foreach($blocks as $block)
                            <option value="{{ $block }}" {{ (old('block', $detail->block) == $block) ? 'selected' : '' }}>{{ $block }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="bloc_troll" class="form-control{{ $errors->has('bloc_troll') ? ' is-invalid' : '' }}" name="bloc_troll">
                        ...
                        @foreach($bloc_trolls as $bloc_troll)
                            <option value="{{ $bloc_troll }}" {{ (old('bloc_troll', $detail->bloc_troll) == $bloc_troll) ? 'selected' : '' }}>{{ $bloc_troll }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="photocrophy" class="form-control{{ $errors->has('photocrophy') ? ' is-invalid' : '' }}" name="photocrophy">
                        ...
                        @foreach($photocrophys as $photocrophy)
                            <option value="{{ $photocrophy }}" {{ (old('photocrophy', $detail->photocrophy) == $photocrophy) ? 'selected' : '' }}>{{ $photocrophy }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="photo_color" class="form-control{{ $errors->has('photo_color') ? ' is-invalid' : '' }}" name="photo_color">
                        ...
                        @foreach($photo_colors as $photo_color)
                            <option value="{{ $photo_color }}" {{ (old('photo_color', $detail->photo_color) == $photo_color) ? 'selected' : '' }}>{{ $photo_color }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="polycarbonate" class="form-control{{ $errors->has('polycarbonate') ? ' is-invalid' : '' }}" name="polycarbonate">
                        ...
                        @foreach($polycarbonates as $polycarbonate)
                            <option value="{{ $polycarbonate }}" {{ (old('polycarbonate', $detail->polycarbonate) == $polycarbonate) ? 'selected' : '' }}>{{ $polycarbonate }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="poly_break" class="form-control{{ $errors->has('poly_break') ? ' is-invalid' : '' }}" name="poly_break">
                        ...
                        @foreach($polybreaks as $poly_break)
                            <option value="{{ $poly_break }}" {{ (old('poly_break', $detail->poly_break) == $poly_break) ? 'selected' : '' }}>{{ $poly_break }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="color_white" class="form-control{{ $errors->has('color_white') ? ' is-invalid' : '' }}" name="color_white">
                        ...
                        @foreach($color_whites as $color_white)
                            <option value="{{ $color_white }}" {{ (old('color_white', $detail->color_white) == $color_white) ? 'selected' : '' }}>{{ $color_white }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="colored_score" class="form-control{{ $errors->has('colored_score') ? ' is-invalid' : '' }}" name="colored_score">
                        ...
                        @foreach($colored_scores as $colored_score)
                            <option value="{{ $colored_score }}" {{ (old('colored_score', $detail->colored_score) == $colored_score) ? 'selected' : '' }}>{{ $colored_score }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="watering" class="form-control{{ $errors->has('watering') ? ' is-invalid' : '' }}" name="watering">
                        ...
                        @foreach($waterings as $watering)
                            <option value="{{ $watering }}" {{ (old('watering', $detail->watering) == $watering) ? 'selected' : '' }}>{{ $watering }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select id="yu_vie" class="form-control{{ $errors->has('yu_vie') ? ' is-invalid' : '' }}" name="yu_vie">
                        ...
                        @foreach($yu_vies as $yu_vie)
                            <option value="{{ $yu_vie }}" {{ (old('yu_vie', $detail->yu_vie) == $yu_vie) ? 'selected' : '' }}>{{ $yu_vie }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row rx">
                ...
                <div class="col-md-6">
                    <?php $addPro = json_decode($detail->add); ?>
                    <select class="form-control{{ $errors->has('add') ? ' is-invalid' : '' }}" multiple name="add[]">
                        @foreach($adds as $add)
                            <option value="{{ $add }}" {{ (is_array(old('add', $addPro)) && in_array($add, old('add', $addPro))) ? 'selected' : '' }}>{{ $add }}</option>
                        @endforeach
                    </select>
                            ...
                            <table class="table">
                                ...
                                @foreach($group as $key => $gp)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select class="form-control" name="size[{{ $key }}]">
                                                ...
                                                @foreach($sizes as $size)
                                                    <option value="{{ $size }}" {{ ($size_st == $size) ? 'selected' : '' }}>{{ $size }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <input type="number" min="0" class="form-control" name="purchase_price[{{ $key }}]" value="{{ ($purchase_price) ? $purchase_price : 0 }}">
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
                    ...
                    <div class="slime_uploadfile" style="width: 100%;">
                        <div class="slim"
                            ...
                            @if(old('image', $request->image))
                                <img src="{{ old('image', $request->image) }}" alt=""/>
                            @endif
    ...
    <script>
        ...
        @if(old('type', $detail->type) && old('type', $detail->type) == "Stock تک دید")
            $('.rx').css('display', 'none');
            $('.stock').css('display', 'block');
            $('.rx input').attr("disabled", 'disabled');
            $('.rx select').attr("disabled", 'disabled');
            $('.stock input').removeAttr("disabled");
            $('.stock select').removeAttr("disabled");
        @elseif(old('type', $detail->type) && old('type', $detail->type) != "Stock تک دید")
            $('.stock').css('display', 'none');
            $('.rx').css('display', 'block');
            $('.stock input').attr("disabled", 'disabled');
            $('.stock select').attr("disabled", 'disabled');
